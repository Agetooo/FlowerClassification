...
def predict():
    if "image" not in request.files:
        return jsonify({"error": "Khong tim thay file anh tai len."}), 400

    file = request.files["image"]
    extractor_type = request.form.get("extractor", "HOG")
    classifier_type = request.form.get("classifier", "Softmax")

    if file.filename == "":
        return jsonify({"error": "Ten tep tin trong."}), 400
    ...
    try:
        ...
        if not model_exists:
            return jsonify(
                {
                    "status": "not_trained",
                    "message": f"Mo hinh cho cau hinh {extractor_type} + {classifier_type} chua duoc huan luyen.",
                }
            )

        img = cv2.imread(temp_path)
        if img is None:
            return jsonify({"error": "Khong the giai ma tep anh dau vao."}), 400
